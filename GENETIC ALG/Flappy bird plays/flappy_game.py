...
def distance_from_pipe(pp, bird):
    pipe_y = pp.top_height_px + Bird.HEIGHT * 2

    y_distance = abs(pipe_y - bird.y)
    x_distance = abs((pp.x) - bird.x)

    y_top = abs(bird.y - 0)
    y_bottom = abs(bird.y - WIN_HEIGHT)

    return x_distance, y_distance, y_top, y_bottom
...
def load_images():
    ...
    return {'background': load_image('background.png'),
            ...
            'babi': load_image('babi.png'),
            'denisa' : load_image('denisa.png'),
            'guta' :  load_image('guta.png'),
            'minune' : load_image('minune.png'),
            'mercedes' : load_image('mercedes.png')
            }
...
def play(agent_list):
    ...
    # manelisti
    manelisti = ['babi', 'minune', 'guta', 'mercedes', 'babi', 'denisa']

    birds = []
    for i, agent in enumerate(agent_list):
        manelist = np.random.randint(6)
        bird = Bird(50, int(WIN_HEIGHT/2 - Bird.HEIGHT/2), 2,
                    (images[manelisti[manelist]]))
        birds.append({"bird" : bird, "alive" : True})
        agent["cost"] = 0
    ...
    while not done:
        ...
        delay += 1
        if delay % 4 == 0: # 4 e corect
            for i, agent in enumerate(agent_list):
                if birds[i]["alive"]:
                    x_distance, y_distance, _, _ = distance_from_pipe(pp, birds[i]["bird"])
                    #x_distance = x_distance / WIN_WIDTH
                    #y_distance = y_distance / WIN_HEIGHT
                    inp = torch.tensor([x_distance, y_distance])
                    out = agent["model"](inp.view(1, -1))
                    if out > 0.5:
                        birds[i]["bird"].msec_to_climb = Bird.CLIMB_DURATION
        ...
        #        done = True
        #        break
        #    elif e.type == KEYUP and e.key in (K_SPACE, K_p):
        #        paused = not paused
        ...
        for i, bird in enumerate(birds):
            pipe_collision = any(p.collides_with(bird["bird"]) for p in pipes)
            if pipe_collision or 0 >= bird["bird"].y or bird["bird"].y >= WIN_HEIGHT - Bird.HEIGHT:
                bird["alive"] = False
                agent_list[i]["cost"] -= pp.x
        ...
        pygame.draw.circle(display_surface, (255, 0, 0), (int(pipe_x), int(pipe_y)), 5)

        for bird in birds:
            if bird["alive"]:
                bird["bird"].update()
                display_surface.blit(bird["bird"].image, bird["bird"].rect)
...
if __name__ == '__main__':
    ...
    # TO BE REMOVED
    #agent = {"model" : Agent(), "score" : 0}
    #play(agent)
    print('Flappy Game Loaded')
